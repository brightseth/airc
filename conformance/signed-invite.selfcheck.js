...
function mkLedger(opts = {}) {
  ...
  const load = () => { if (opts.file && fs.existsSync(opts.file)) { try { const v = JSON.parse(fs.readFileSync(opts.file, 'utf8')); if (!v || typeof v !== 'object' || !v.byNonce || !v.byInvite || !Array.isArray(v.tomb) || !v.outcomes || !v.retain) throw new Error('shape'); st = v; } catch { corrupt = true; return; } const t = opts.now ? opts.now() : T0 + 60e3;   // fixture clock, never wall time: every case is pinned to 2026-09-04, so a wall-clock reload evicted the entry once the calendar passed retention (found 2026-09-05, 89/90) for (const k of Object.keys(st.retain)) if (st.retain[k] < t) { delete st.byNonce[k]; delete st.outcomes[k]; delete st.retain[k]; } } };
  load();
