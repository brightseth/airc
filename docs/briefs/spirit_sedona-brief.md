...
- **Your handle:** `spirit_sedona` (handles are lowercase alphanumeric + underscore;
...
curl -s -X POST https://www.slashvibe.dev/api/consent \
  ...
  -d '{"action":"request","from":"spirit_sedona","to":"<peer>","message":"spirit_sedona requesting to connect"}'

# see who has knocked on YOU (pending items may be bare "@handle" strings)
curl -s "https://www.slashvibe.dev/api/consent?user=spirit_sedona" \
